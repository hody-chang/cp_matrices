function [results, diagnostics] = example_l_shaped_manifold_heat_refinement_angle_rotation(opts)
%EXAMPLE_L_SHAPED_MANIFOLD_HEAT_REFINEMENT_ANGLE_ROTATION
% Angle-rotation CPM heat equation on a 90 degree corner.
%
% This is basically the same calculation as the V-shaped example, but the
% geometry is deliberately simpler.  Branch A is the vertical line segment
% from (1,1) to (1,2).  Branch B is the horizontal line segment from (1,1)
% to (2,1).  The shared vertex is (1,1), so the opening angle is 90 degrees.
%
% I keep the heat equation, the initial condition, and the endpoint boundary
% condition the same.  Only the manifold is changed.
%
% The shared vertex itself is advanced by the symmetric two-neighbor formula
%
...
%       example_l_shaped_manifold_heat_refinement_angle_rotation();
%
% The default run uses h = 1./[200 400 800].  Figure 1 compares u(s,t)
% against the exact solution on the finest grid.  Figure 2 plots L_inf and
% L_2 errors versus h.  Figure 3 plots final-time |u_h - u| versus s.
  ...
  if showProgress
    fprintf(['L-shaped angle-rotation CPM run: %d angle(s), %d h value(s), ' ...
  ...
  for ia = 1:numel(anglesDeg)
    if showProgress
      fprintf('Corner angle %d/%d: alpha = %.6g degrees\n', ...
    ...
    for ih = 1:numel(hvals)
      ...
      if showProgress
        fprintf(['  Finished corner alpha = %.6g, h = %.6g: steps = %d, ' ...
                 'FinalInfError = %.4e\n'], ...
...
%BUILDBRANCH Build one branch-local CPM band and geometry.
  ...
  vertexGridMask = makeVertexGridMask(xBand, yBand, vertex);
...
function [xRot, yRot] = rotatePoints(x, y, theta, vertex)
%ROTATEPOINTS Rotate points about the shared vertex.
  ...
  xShift = x - vertex(1);
  yShift = y - vertex(2);
  xRot = vertex(1) + c * xShift - s * yShift;
  yRot = vertex(2) + s * xShift + c * yShift;
end
...
function mask = makeVertexMask(cpx, cpy, sBand, h, vertexTol, vertex)
%MAKEVERTEXMASK Select band points whose closest point is the shared vertex.
  ...
  cpScale = max([1; abs(cpx(:)); abs(cpy(:)); abs(vertex(:))]);
  mask = mask | (hypot(cpx - vertex(1), cpy - vertex(2)) <= ...
                 100 * eps(cpScale));
  if ~any(mask)
    [~, idx] = min(abs(sBand));
    mask = false(size(sBand));
...
function mask = makeVertexGridMask(xBand, yBand, vertex)
%MAKEVERTEXGRIDMASK Select the actual Cartesian grid point at the vertex.

  scale = max([1; abs(xBand(:)); abs(yBand(:)); abs(vertex(:))]);
  mask = hypot(xBand - vertex(1), yBand - vertex(2)) <= 100 * eps(scale);
  if ~any(mask)
    [~, idx] = min(hypot(xBand - vertex(1), yBand - vertex(2)));
...
%BRANCHCOORDINATES Map branch coordinate s to the embedded L shape.

  unusedAngle = angleDeg; %#ok<NASGU>
  if side < 0
    x = ones(size(s(:)));
    y = 1 - s(:);
  else
...
%PLOTANGLESOLUTIONS Plot final u(s,t) for each angle on the finest grid.
  ...
  for ia = 1:numel(anglesDeg)
    ...
    xlabel(ax, 's');
    ylabel(ax, 'u');
    title(ax, sprintf('corner angle = %g^\\circ, h = %.4g', ...
  ...
  title(layout, sprintf('L-shaped angle-rotation heat solution at t = %.4g', ...
...
%PLOTCONVERGENCEBYANGLE Plot L_inf and L_2 final errors versus h.
  ...
  for ia = 1:numel(anglesDeg)
    ...
    xlabel(ax, 'h');
    ylabel(ax, 'error');
    title(ax, sprintf('corner angle = %g^\\circ', anglesDeg(ia)));
    if ia == 1
      if numel(h) >= 2
  ...
  title(layout, 'L-shaped final-time error convergence');
end
